...
export default function Login() {
  ...
  // Once a session exists (sign-in or OAuth redirect), go to the dashboard.
  React.useEffect(() => {
  ...
  const submit = async (ev: React.FormEvent) => {
    ...
    try {
      if (mode === "signin") {
        ...
        // navigation handled by the useEffect above once the session lands
      } else {
        await signUp(email, password, role, email.split("@")[0]);
        setNotice(
          "Account created! If email confirmation is enabled in Supabase, check your inbox — otherwise sign in now.",
        );
        setMode("signin");
      }
    } catch (err) {
        ...
        >
          ...
          <p className="mt-1 text-sm text-muted-foreground">
            {mode === "signin"
              ...
              : "Set up access to your TransitOps workspace"}
          </p>

          {!isSupabaseConfigured && (
            <div className="mt-4 flex items-start gap-2 rounded-lg border border-amber-500/30 bg-amber-500/10 p-3 text-xs text-amber-600 dark:text-amber-400">
          ...
          {formError && (
            <div className="mt-4 flex items-start gap-2 rounded-lg border border-destructive/30 bg-destructive/10 p-3 text-xs font-medium text-destructive">
              <AlertCircle className="mt-0.5 size-4 shrink-0" />
              <span>{formError}</span>
            </div>
          )}
          {notice && (
            <div className="mt-4 flex items-start gap-2 rounded-lg border border-emerald/30 bg-emerald/10 p-3 text-xs font-medium text-emerald">
          ...
          <form onSubmit={submit} className="mt-6 space-y-5">
            ...
            <div className="space-y-2">
              <Label>{mode === "signup" ? "Account role" : "Sign in as"}</Label>
              ...
              {mode === "signin" && (
                <p className="text-[11px] text-muted-foreground">
                  Role is set when the account is created.
                </p>
              )}
            </div>
